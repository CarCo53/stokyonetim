...
router.post('/', auth, role(['admin']), (req, res) => {
    const { name, address, firm_id } = req.body;
    if (!name || !firm_id) return res.status(400).json({ message: 'Mağaza adı ve firma ID zorunlu.' });
    const db = new sqlite3.Database(dbFile);
    db.run(
        'INSERT INTO stores (name, address, firm_id) VALUES (?, ?, ?)',
        [name, address || null, firm_id],
        function (err) {
            db.close();
            if (err) return res.status(400).json({ message: 'Mağaza eklenemedi.', detail: err.message });
...
router.put('/:id', auth, role(['admin']), (req, res) => {
    ...
    // firm_id güncellemesi opsiyonel kalsın, verilirse güncellenir
    let sql, params;
    if (firm_id) {
        sql = 'UPDATE stores SET name = ?, address = ?, firm_id = ? WHERE id = ?';
        params = [name, address || null, firm_id, req.params.id];
    } else {
        sql = 'UPDATE stores SET name = ?, address = ? WHERE id = ?';
        params = [name, address || null, req.params.id];
    }
