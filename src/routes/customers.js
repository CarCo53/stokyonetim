...
router.post('/', auth, role(['admin', 'patron', 'çalışan']), (req, res) => {
    ...
    db.run(
        'INSERT INTO customers (store_id, name, surname, birthdate, tckn, consent_personal_data) VALUES (?, ?, ?, ?, ?, ?)',
        [store_id, name, surname, birthdate || null, tckn || null, Number(consent_personal_data)],
        function (err) {
            if (err) {
                db.close();
...
// Herkes SADECE kendi mağazasındaki müşteriyi güncelleyebilir. Admin tüm müşterileri güncelleyebilir.
router.put('/:id', auth, role(['admin', 'patron', 'çalışan']), (req, res) => {
    ...
    db.get('SELECT * FROM customers WHERE id = ?', [id], (err, customer) => {
        ...
        db.run(
            'UPDATE customers SET name=?, surname=?, birthdate=?, tckn=?, consent_personal_data=? WHERE id=?',
            [name, surname, birthdate || null, tckn || null, Number(consent_personal_data), id],
            function (err) {
                db.close();
